document.addEventListener("DOMContentLoaded", function(){
    ...
    const maxRadius = 40, minRadius = 8;
    ...
    function Circle(x, y, radius, color){
        ...
        this.radius = radius;
        this.color = color;
        this.velocity = { // tao van toc random tu -2 toi 2
            x: Math.random()*4-2,
            y: Math.random()*4-2
        }
    }
    ...
    // tao va cham
    Circle.prototype.colission = function(){
        if(this.x >= canvas.width || this.x <= 0){
            this.velocity.x = -this.velocity.x; // cho chay nguoc lai
        }
        if(this.y >= canvas.height || this.y <= 0){
            this.velocity.y = -this.velocity.y; // cho chay nguoc lai
        }
    }
    ...
    // tao chuyen dong
    function animate(){
        window.requestAnimationFrame(animate); // de quy ham animate 60 lan 1 giay
        c.clearRect(0, 0, canvas.width, canvas.height);
        arrayCirle.forEach(circle => { // duyet tung phan tu trong mang Circle
            // circle.draw();
            circle.update();
        });
    }
